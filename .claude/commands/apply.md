...
- Extract: **company name**, **role title**, **department** (if mentioned), **location**, and **language** of the posting (Croatian, Dutch, English, or another language).
...
- **Match the language of the job posting only if the candidate profile supports that working language** (Croatian posting -> Croatian cover letter, Dutch posting -> Dutch cover letter, English posting -> English cover letter). If the posting is Croatian/Dutch but the candidate has not claimed professional proficiency, ask before writing a non-English letter; default to English and flag the language gap.
...
**3. Keyword coverage.** Reuse the required/preferred keyword list you extracted in Step 1 — do not re-derive it. Match each keyword against the extracted text, **in the posting's language**. If a Croatian or Dutch posting uses local terms while the CV is in English, count legitimate English equivalents as synonym-only and note the language difference. Report a table:
